...
def get_muscle_group_time user_id, time_interval = 30
    results = exec_sql("SELECT primary_muscle_group, SUM(duration_seconds) FROM exercise_sets LEFT JOIN exercises ON exercise_sets.exercise_id = exercises.exercise_id WHERE user_id = #{user_id} GROUP BY primary_muscle_group;").to_a
    output = {}
...
def get_user_exercises user_id
    exercises = exec_sql("SELECT exercise_id FROM exercise_sets WHERE user_id = #{user_id} GROUP BY exercise_id;").to_a
    output = []
